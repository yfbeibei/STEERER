...
class Baseline_Counter(nn.Module):
    ...
    def forward(self,inputs, labels=None, mode='train'):
        if self.config.counter_type == 'single_resolution':
            ...
            in_list = self.backbone(inputs) #inputs=[1,3,768,1024]
            ...
            #in_list[0]=[1,48,192,256],in_list[1]=[1,96,96,128],in_list[2]=[1,192,48,64],in_list[3]=[1,384,24,32]
            
            out_list =self.upsample_module(in_list,self.multi_counters,self.counter_copy)
            
            #[1,1,768,1024],[1,1,384,512],[1,1,192,256],[1,1,96,128]
            ...
            labels = labels[self.label_start:self.label_end]#[6,768,768],[6,384,384],[6,192,192],[6,96,96]
            # import pdb
            # pdb.set_trace()

            for i, label in enumerate(labels):
                label_list.append(self.gaussian(label.unsqueeze(1))*self.weight)#[6,1,768,768],[6,1,384,384],[6,1,192,192],[6,1,96,96]
            ...
            moe_label,score_gt = self.get_moe_label(out_list, label_list, self.route_size)#[6,1,3,3],[6,4,3,3]

            # import pdb
            # pdb.set_trace()           
            mask_gt = torch.zeros_like(score_gt) #[6,4,3,3]

            if mode =='train' or mode =='val':
                mask_gt = mask_gt.scatter_(1,moe_label, 1) #[6,4,3,3]

            loss_list = []
            outputs = torch.zeros_like(out_list[0]) #[6,1,768,768]
            label_patch = torch.zeros_like(label_list[0]) #[6,1,768,768]

            result.update({'acc1': {'gt':0, 'error':0}})

      
            mask_add = torch.ones_like(mask_gt[:,0].unsqueeze(1)) #[6,1,3,3]
            for i in range(mask_gt.size(1)):

                kernel = (int(self.route_size[0] / (2 ** i)), int(self.route_size[1] / (2 ** i)))#(256,256),(128,128),(64,64),(32,32)
                loss_mask = F.upsample_nearest(mask_add,   size=(out_list[i].size()[2:])) #[6,1,768,768]
                ...
                if i == 0:
                    ...
                    label_patch = label_patch.transpose(2, 1).view(B_, L_, kernel[0], kernel[1]) #[6,9,256,256]
                else:
                    gt_slice  = F.unfold(label_list[i], kernel,stride=kernel) #[6,16384,9],[6,4096,9],[6,1024,9]
                    B, KK, L = gt_slice.size()

                    pick_gt_idx = (moe_label.flatten(start_dim=1) == i).unsqueeze(2).unsqueeze(3) #[6,9,1,1]
                    gt_slice = gt_slice.transpose(2,1).view(B, L,kernel[0], kernel[1]) #[6,9,128,128],[6,9,64,64],[6,9,32,32]
                    pad_w, pad_h =(self.route_size[1] - kernel[1])//2, (self.route_size[0] - kernel[0])//2#(64,64),(96,96),(112,112)
                    gt_slice = F.pad(gt_slice, [pad_w,pad_w,pad_h,pad_h], "constant", 0.2)#[6,9,256,256]
                    gt_slice = (gt_slice * pick_gt_idx)
                    label_patch += gt_slice#[6,9,256,256]
            ...
            label_patch =label_patch.view(B_num, patch_h*patch_w, -1).transpose(1,2) #[6,65536,9]
            label_patch = F.fold(label_patch, output_size=(H_num, W_num), kernel_size=self.route_size, stride=self.route_size) #[6,1,768,768]
            ...
            if mode =='train' or mode =='val':
                ...
                result['pre_den'].update({'1':out_list[0]/self.weight}) #[6,1,768,768]
                result['pre_den'].update({'8': out_list[-1]/self.weight}) #[6,1,96,96]
                result['gt_den'].update({'1': label_patch/self.weight}) #[6,1,768,768]
                result['gt_den'].update({'8': label_list[-1]/self.weight}) #[6,1,96,96]
